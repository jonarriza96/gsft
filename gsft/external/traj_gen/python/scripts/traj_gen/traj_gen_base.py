...
class TrajGen(object):
    ...
    def addPin(self, pin):
        ...
        assert (
            pin["t"] >= self.Ts[0] and pin["t"] <= self.Ts[-1]
        ), "t of this pin is out of range of knots"
    ...
    def showPath(self, fig_title):
        assert (
            self.dim >= 2 and self.dim < 4
        ), "Here you can only show the path in 2/3D."
        # rc("text", usetex=True)
        fig = plt.figure()
        if self.dim == 3:
        ...
        for pin in self.pinSet:
            if pin["d"] == 0:
                ...
                if len(X_.shape) == 2:
                    ...
                    if self.dim == 2:
                        ax.add_patch(
                            patches.Rectangle((x_, y_), x_size_, y_size_),
                            facecolor="r",
                            alpha=0.1,
                        )
                    ### 3D cube/bar
                        ...
                        ax.bar3d(
                            x_, y_, z_, x_size_, y_size_, z_size_, color="r", alpha=0.1
                        )
                else:
                    ## fix pin
                    ### 2D
    ...
    def showTraj(self, plotOrder):
        ...
        for dd in range(self.dim):
            for pin in self.pinSet:
                ...
                X_ = pin["X"]
                axs[dd, d_].vlines(
                    t_, -10.0, 10, color="k", linestyle="dashed", linewidth=0.5
                )
                if len(X_.shape) == 2:
                    # loose pin
                    axs[dd, d_].errorbar(
            ...
            for d in range(plotOrder + 1):
                ...
                for dd in range(self.dim):
                    ...
                    for t_ in self.Ts:
                        axs[dd, d].vlines(
                            x=t_,
                            ymin=np.min(Xs[dd]) - 0.5,
                            ymax=np.max(Xs[dd]) + 0.5,
                            color="k",
                            linestyle="dashed",
                            linewidth=0.3,
                        )
