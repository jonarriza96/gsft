...
if __name__ == "__main__":
    ...
    optTraj.addPin(pin_)
    passCube = np.array([[3.0, 4.2], [-3.0, -2.0], [1.0, 1.5]])  # in shape [dim, xxx]
    pin_ = {"t": 3, "d": 1, "X": passCube}
    optTraj.addPin(pin_)
    ...
    optTraj.showTraj(4)
    print("path")
    fig_title = "minimzed derivatives order: {}".format(
        np.where(optTraj.weight_mask > 0)[0].tolist()
    )
