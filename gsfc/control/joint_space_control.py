...
from gsfc.utils.panda import panda_limits
...
def QP_term(
    ad, J, H, J_dot, Jm, q, qd, qdd, dt, e3_ee, acc_ee_norm, solver="clarabel", base="0"
):
    # x = [qdd, sl, esf, qd, q]
    ...
    Aeq[:6, :7] = J  # qdd
    if controller["slack"]:
        Aeq[:6, 7:13] = np.eye(6)  # slacks
    beq[:6] = ad - J_dot @ qd
    if controller["task_space"] == "gc":
        beq[:3] -= np.array([0, 0, 9.81])

    Aeq[6, 7:10] = e3_ee / acc_ee_norm
    Aeq[6, 13] = 1
    beq[6] = 1 - np.dot(ad[:3] + np.array([0, 0, 9.81]), e3_ee) / acc_ee_norm
    ...
    # Weighting matrices
    Qw = np.eye(7 + 6 + 1 + 14)  # Quadratic component of objective function
    cw = np.zeros((7 + 6 + 1 + 14,))  # Linear component of objective function
    ...
    q_lb = panda_limits["q_min"]  # * 1.5
    q_ub = panda_limits["q_max"]  # * 1.5
    lb = np.hstack((qdd_lb, sl_lb, esf_lb, qd_lb, q_lb))
    ub = np.hstack((qdd_ub, sl_ub, esf_ub, qd_ub, q_ub))
...
def SOCP(ad, e3_ee, J, H, J_dot, Jm, q, qd, qdd, dt):
    # TODO:
    #   - Add slosh-free error

    n = 7 + 7 + 7 + 6 + 1 + (1 + 1 + 1)  # number of decision variables
    p = 6 + 7 + 7  # number of equality constraints
    ...
    F[:6, :7] = J  # qdd
    if controller["slack"]:
        F[:6, 21:27] = np.eye(6)  # slacks
    g[:6] = ad - J_dot @ qd

    F[6:13, :7] = -dt * np.eye(7)
    F[6:13, 7:14] = np.eye(7)
    g[6:13] = qd

    F[13:20, 7:14] = -dt * np.eye(7)
    F[13:20, 14:21] = np.eye(7)
    g[13:20] = q + dt * qd
    ...
    for j in range(7):
        ...
        ci = np.zeros((n))
        ci[j] = -1
        di = panda_limits["qdd_max"][j]
        soc_constraints += [cp.SOC(ci.T @ x + di, Ai @ x + bi)]
        ...
        ci = np.zeros((n))
        ci[14 + j] = -1
        di = panda_limits["q_max"][j]
        soc_constraints += [cp.SOC(ci.T @ x + di, Ai @ x + bi)]

        # 1.6 - q min
        ci = np.zeros((n))
        ci[14 + j] = 1
        di = -panda_limits["q_min"][j]
        soc_constraints += [cp.SOC(ci.T @ x + di, Ai @ x + bi)]

        # 1.7 - qddd max
        ci = np.zeros((n))
        ci[j] = -1
        di = qdd[j] + dt * panda_limits["qddd_max"][j]
        soc_constraints += [cp.SOC(ci.T @ x + di, Ai @ x + bi)]
    ...
    # 2 - Slosh-free equality --> |a_ee| < esf_bar + dot(a_ee, e3_ee), where a_ee = (ad + sl) + g
    Ai = np.zeros((3, n))
    bi = np.zeros((3))
    ci = np.zeros((n))
    di = np.dot(ad[:3] + np.array([0, 0, 9.81]), e3_ee)
    Ai[:, 21:24] = 1
    bi[:] = ad[:3] + np.array([0, 0, 9.81])
    ci[21:24] = e3_ee
    ci[27] = 1
    soc_constraints += [cp.SOC(ci.T @ x + di, Ai @ x + bi)]

    # 3 - Cost function bound
    ...
    prob.solve()
    if prob.status == cp.INFEASIBLE or prob.status == cp.UNBOUNDED:
        print("CVXPY status:", prob.status)
        raise RuntimeError("SOCP failed!")

    cost = prob.value
    sol = x.value

    ###### Make it compatible with current pipeline
    qdd = sol[:7]
    qd = sol[7:14]
    q = sol[14:21]
    sl = sol[21:27]
    esf = sol[27]
    sol = np.hstack((qdd, sl, qd, q, esf))
    # print(sl)
    ######
